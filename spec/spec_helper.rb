...
RSpec.configure do |config|
  config.before(:each) do
    stub_request(:get, /api.freecurrencyapi.com\/v1\/currencies/).
      with(headers: { "Accept" => "*/*", "User-Agent" => "Ruby" }).
      to_return(status: 200, body: api_response, headers: { "Content-Type" => "application/json" })

    stub_request(:get, /api.freecurrencyapi.com\/v1\/currencies/).
      with(headers: { "Connection" => "close", "Host" => "api.freecurrencyapi.com", "User-Agent" => "http.rb/#{HTTP::VERSION}" }).
      to_return(status: 200, body: api_response, headers: { "Content-Type" => "application/json" })
  end
end

path_to_file = Dir.pwd + "/spec/support/usd_to_eur.json"
usd_to_eur = open(path_to_file).read.chomp
RSpec.configure do |config|
  config.before(:each) do
    stub_request(:get, /api.freecurrencyapi.com\/v1\/latest/).
      with(headers: { "Accept" => "*/*", "User-Agent" => "Ruby" }).
      to_return(status: 200, body: usd_to_eur, headers: { "Content-Type" => "application/json" })
